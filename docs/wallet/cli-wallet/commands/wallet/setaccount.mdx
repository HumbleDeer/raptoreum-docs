---
sidebar_position: 44
---
import ReactPlayer from 'react-player'

# setaccount

*setaccount "address" "account"*

DEPRECATED. Sets the account associated with the given address.

```bash
raptoreum-cli setaccount "XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwG" "tabby"
```

<ReactPlayer playing controls url={[
    {src: `${require('./assets/setaccount.webm').default}`, type: 'video/webm'}]} />

## Arguments

| Argument | Type | Description |
|--|--|--|
| "address" | (string) | The raptoreum address to send to. |

```bash
Arguments:
1. "address"         (string, required) The raptoreum address to be associated with an account.
2. "account"         (string, required) The account to assign the address to.
       "CONSERVATIVE"
```

## Output

| Result | Type | Description |
|--|--|--|
| ... | ... | ... |

```bash
Result:
{
  ...
}
```

## Examples

```bash
curl --user myusername --data-binary '{"jsonrpc": "1.0", "id":"curltest", "method": "setaccount", "params": ["XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwG", "tabby"] }' -H 'content-type: text/plain;' http://127.0.0.1:9998/
```

