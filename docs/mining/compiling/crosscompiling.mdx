...
This guide will show you how to automatically cross compile EXE files for the Raptoreum CPU miner through a tool that utilizes Ubuntu and WSL.

:::

The full offline guide as well as files needed for this guide are here: [Cross Compile Files](./assets/cross-compile-rtm-miner.zip)
...
1. Open cross-compile-rtm-miner.ps1 by right clicking on the file and selecting `Run With PowerShell`. A GUI will pop up. This GUI gives you access to everything you need to cross compile the Raptoreum CPU miner (based on `cpuminer-opt`)
