...
Using any other OS may result in unexpected behavior and [PoSe][] bans.

Operating system requirements are fairly simple. 

Some installation scripts use Aptitude for managing packages, and will thus not function without. Debian, Ubuntu, and related distributions of Debian are examples of distributions utilizing Aptitude. Installing dependencies via Aptitude is only needed if building Raptoreum Core from source. When using the pre-built binaries it should not be necessary.
